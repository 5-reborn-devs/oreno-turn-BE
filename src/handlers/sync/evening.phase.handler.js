...
export const eveningDrawHandler = async(room) => {
  const failCode = getFailCode();
      ...
      //페이즈 넘어갈때 리롤 고민 필요.
      // 개인에게 리스폰스
      sendResponsePacket(client, PACKET_TYPE.EVENING_DRAW_NOTIFICATION, {EveningDistributionNotification});
...
export const eveningPickHandler = async(socket, payloadData)=>{
  ...
  try{
    ...
    //리스폰스 슛
    sendResponsePacket(socket, PACKET_TYPE.EVENING_DRAW_RESPONSE, { EveningDrawResponse });
